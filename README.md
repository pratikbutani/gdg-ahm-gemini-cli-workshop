# gdg-ahm-gemini-cli-workshop
GDG Ahmedabad - Gemini CLI Workshop by Pratik Butani &amp; Ravi Rupareliya

1. Act as a senior product owner and Help me prepare mvp for this requirement.. 
...
2. I want to create a mockup design for this mvp version, help me create prompt for the same. I will utilise that prompt in Google Stitch. No need to provide image sample.


3. I would like to implement a post generation in @code.html where Gemini API {api-key} will be used and based on selected event we will generate the linkedIn post. Use gemini-2.5-flash model


4. Follow the instructions from @instructions.md and populate the selector with appropriate values. Use the description of the selected value as the hidden base context for LinkedIn post generation. Do not reveal these details to the user. If the user provides any input, treat it as a personalized context and blend it with the base context.
